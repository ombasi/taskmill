...
def deposit():
    ...
    try:
        ...
        payment_settings = active_deposit_methods(current_user)
    except Exception:
    ...
    try:
        recent_deposits = (
            Deposit.query.filter_by(user_id=current_user.id)
            .order_by(Deposit.created_at.desc()).limit(8).all()
        )
    except Exception:
        pass
...
def withdraw():
    ...
    if request.method == "POST":
        ...
        # KYC light for large withdrawals (UGX threshold)
        try:
            from models.settings import Settings
            _s = Settings.query.first()
            kyc_threshold = float(getattr(_s, "kyc_withdraw_threshold", None) or 500000)
        except Exception:
            kyc_threshold = 500000.0
        try:
            ugx_amt = float(amount_ugx if "amount_ugx" in dir() else amount)
        except Exception:
            ugx_amt = float(request.form.get("amount") or 0)
        # amount may already be UGX after conversion in route
        try:
            from utils.fx import to_ugx
            # prefer converted variable if exists in local scope later - use form
            pass
        except Exception:
            pass
        if float(getattr(current_user, "available_balance", 0) or 0) >= 0:
            # check requested amount from form local currency converted earlier in this function
            pass
        _req = None
        for _name in ("amount_ugx", "amount"):
            if _name in dir():
                try:
                    _req = float(locals().get(_name) or 0)
                except Exception:
                    pass
        # safer: read from form after conversion block — inject simpler check using request
        try:
            from utils.fx import to_ugx as _to_ugx
            _raw = float(request.form.get("amount") or 0)
            _req_ugx = _to_ugx(_raw, getattr(current_user, "currency", None) or "UGX")
        except Exception:
            _req_ugx = float(request.form.get("amount") or 0)
        if _req_ugx >= kyc_threshold and (getattr(current_user, "kyc_status", None) or "None") != "Approved":
            flash("Large withdrawals require verified ID. Submit KYC first.", "warning")
    ...
    try:
        from utils.user_guidance import normalize_checklist
        checklist_items = normalize_checklist(checklist)
    except Exception:
